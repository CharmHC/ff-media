...
> **Status:** Living document · **Version:** 0.29 · **Last updated:** 2026-07-15
>
...
- **A delta needs the PREVIOUS release present at pack time.** `vpk pack` builds a delta by diffing
  ...
  a delta was built. Measured: **190.8 MB full → 18.6 MB delta (≈90 % smaller).** **v1.2.0 is the first
  release to actually ship a delta from CI** (the fix landed after v1.1.1, so v1.1.0/v1.1.1 had none):
  the workflow logged `Building delta 1.1.1 -> 1.2.0` — *9 files patched, 2 new, 432 unchanged (excluded)* —
  and the release carries both `FFMedia-1.2.0-full.nupkg` and `FFMedia-1.2.0-delta.nupkg`, so an existing
  v1.1.1 user updates with a **sub-1 MB** download instead of 191 MB. The v0.22 fix is now proven in
  production, not just in a local dry-run.
...
| Date | Version | Change |
|---|---|---|
| 2026-07-15 | 0.29 | **Released `v1.2.0` — the biggest release since 1.0, and the first to ship a real delta.** Tagged `v1.2.0` at `main` (`96002e3`) and pushed; the tag-gated `release.yml` packed and published the GitHub Release. It bundles the two tools that merged since v1.1.1: **the GIF Maker (M8)** and **the video preview with frame capture (M9)**. **Pre-flight from a clean checkout:** clean (`--no-incremental`) Release build **0 warnings / 0 errors**; **822/822** unit tests. **The delta finally shipped from CI** (§15): the workflow logged `Building delta 1.1.1 -> 1.2.0` — *9 patched, 2 new, 432 unchanged* — and the release carries both a full and a `*-delta.nupkg`, so an existing v1.1.1 user updates with a **sub-1 MB** download instead of 191 MB. This is the v0.22 delta fix proven **in production**, not just in the local dry-run it was merged on. Release published non-draft/non-prerelease with house-style notes (feature/fix sections + install/update instructions + bundled-versions footer). **Bundled unchanged: yt-dlp 2026.07.04 · FFmpeg + FFprobe (BtbN win64-gpl).** **Still unverified — and it ships to users in this build:** M9's preview has **never had a human click-through** (the "does the slider visibly advance / does the first frame render / does a preview left playing stop on navigate-away" items — pixels and sound a test cannot judge; the *does-it-open* behaviour is now pinned by real-`MediaElement` integration tests). The cosmetic `<Version>0.9.0</Version>` in `FFMedia.App.csproj` was left as-is — `pack.ps1` overrides it with the tag version, so the shipped build is correctly 1.2.0; the csproj value only affects a dev build's displayed version. Docs-only change — no code touched. |
| 2026-07-15 | 0.28 | **M9 merged to `main` (PR #30), and the click-through fix it needed to actually work.** SDD sync: §17's M9 row moves from *"delivered (branch)"* to **complete**, and records the defect the first human click-through found — **the preview never opened a single video, and 822 green tests said it did.** A `MediaElement` with `LoadedBehavior = Manual` (load-bearing — without it the element autoplays and ignores the transport) **does not begin loading its media when you merely assign `Source`**; it waits to be told to do something. `Open()` assigned `Source` and returned, so **`MediaOpened` never fired**, `LoadAsync` awaited it forever, and `IsReady` stayed false. Fixed: `Open()` now calls `Pause()`, which opens the file and parks it on the first frame (measured on a real message loop: *Source alone → **never opens***; *Source + `Pause()` → opens in ~440 ms*). **Worse, `MediaFailed` could not arrive either — so the VP9/WebM proxy fallback, the entire reason M9 exists, was unreachable in production**, while every proxy test stayed green driving a *fake* player that refuses on cue. **The lesson: "it cannot be tested headlessly" was FALSE and nobody checked** — `WpfHost` already runs a real `Dispatcher.Run()` loop, which is all Media Foundation needs; that inherited assumption was written into the spec, plan, ledger and three handoffs, and **it cost the entire feature**. Now pinned by **two integration tests driving a real `MediaElement` against real files** — an MP4 must **open and report its duration**, a **VP9/WebM must be REFUSED** — both mutation-proven (without the `Pause()` they hang). **Verified:** clean Release build **0 warnings / 0 errors**; **822/822** unit; **14/14** integration (was 12). **Still unverified (pixels & sound, a human's to judge):** whether the readout/slider *visibly advance*, whether the first frame *renders* on load, whether a restored frame *comes back* after a page revisit, and whether a preview left *playing* actually *stops* on navigate-away. Docs-only sync change — no code touched. |
